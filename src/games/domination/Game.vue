...
export default defineComponent({
  ...
  methods: {
    ...
    changePointOwner(pointIndex, ownerIndex) {
      const db = getDatabase();
      const {gameId} = this.$route.params
      const Ref = ref(db, `/games/${gameId}/points/${pointIndex}`);
      runTransaction(Ref, (post) => {
        if (post) {
          post.owner = ownerIndex
        }
        return post
      })
    },
    ...
    tick() {
      ...
      if (this.capture.start) {
        ...
        if (this.capture.progressPercent >= 100) {
          ...
          this.changePointOwner(this.currentPoint, this.capture.team)
